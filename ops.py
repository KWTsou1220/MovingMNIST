...
"""
def batch_norm(x, name=None, is_training=tf.constant(True), epsilon=1e-3, decay=0.999):
    ...
    with vs.variable_scope(name or 'batch_norm'):
        ...
        pop_mean = tf.get_variable('pop_mean', [1, dim], initializer=tf.zeros_initializer, trainable=False)
        pop_var = tf.get_variable('pop_var', [1, dim], initializer=tf.ones_initializer(), trainable=False)
        if len(shape) == 2:
...
def _weight_variable(shape, name='weights', initializer=tf.random_normal_initializer(mean=0., stddev=0.001,)):
